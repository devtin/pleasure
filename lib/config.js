...
const cleanPackageJsonName = (packageJsonName) => {
  return packageJsonName.replace(/^.*?\//, '')
}

const getProjectName = () => {
  const packageJson = path.join(getProjectPath(), 'package.json')
  if (fs.existsSync(packageJson)) {
    return cleanPackageJsonName(require(packageJson).name)
  }
}
...
module.exports = merge({
  ...
  appPath: getAppPath(),
  initFile: getEnvValue('INIT_FILE') || path.join(getAppPath(), 'init.js'),
  mainFile: getEnvValue('MAIN_FILE') || path.join(getAppPath(), 'main.js'),
  api: {
